...
# Create a formatted date for folder structure (YYYY-MM-DD)
if len(sys.argv) > 1:
    # If date was provided via command line, parse it
    FOLDER_DATE = datetime.datetime.strptime(TARGET_DATE, "%Y%m%d").strftime("%Y-%m-%d")
else:
    # If using today's date, format it
...
BASE_OUTPUT_DIR = os.path.join("game_graphics", FOLDER_DATE)
...
def main():
    ...
    print(f"=== Processing games for date: {FOLDER_DATE} ===\n")
